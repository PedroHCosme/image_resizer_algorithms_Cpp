    ...
    for (int y = 0; y < new_height; ++y) {
        for (int x = 0; x < new_width; ++x) {
            for (int c = 0; c < source.spectrum(); ++c) {
                ...
                // Interpolate color value from the source image
                result(x, y, 0, c) = interpolate_color(source, src_x, src_y, c);
            }
        }
    }
 ...
 */
unsigned char resize_bilinear::interpolate_color(const cimg_library::CImg<unsigned char>& source, float x, float y, int channel) const {
    ...
    // Interpolate between the top and bottom pairs of pixels
    ...
    // Interpolate between the top and bottom results
